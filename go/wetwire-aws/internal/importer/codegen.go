...
import (
	...
	"github.com/lex00/wetwire/go/wetwire-aws/resources"
)
...
// replace github.com/lex00/wetwire/go/wetwire-aws => ../path/to/wetwire-aws
`, modulePath)
...
func GenerateCode(template *IRTemplate, packageName string) map[string]string {
	...
	if _, hasMain := categoryCode["main"]; !hasMain {
		if mappingsCode := generateMappings(ctx); mappingsCode != "" {
			...
			imports["github.com/lex00/wetwire/go/wetwire-aws/intrinsics"] = true
...
func buildFile(packageName, description string, imports map[string]bool, code string) string {
	...
	if len(imports) > 0 {
		...
		for _, imp := range sortedImports {
			if imp == "github.com/lex00/wetwire/go/wetwire-aws/intrinsics" {
...
func generateParameter(ctx *codegenContext, param *IRParameter) string {
	...
	ctx.imports["github.com/lex00/wetwire/go/wetwire-aws/intrinsics"] = true
...
func generateResource(ctx *codegenContext, resource *IRResource) string {
	...
	// Add import
	ctx.imports[fmt.Sprintf("github.com/lex00/wetwire/go/wetwire-aws/resources/%s", module)] = true
